document.addEventListener("DOMContentLoaded", () => {
  ...
  btnHistorial.className =
    "w-full mt-4 bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded transition-all duration-300 hidden flex items-center justify-center";
  btnHistorial.innerHTML =
    '<i class="fas fa-history mr-2"></i>Ver Historial de Consultas';
  // Lo insertamos justo después del botón de estudios
  verEstudiosBtn.parentNode.insertBefore(
    btnHistorial,
    verEstudiosBtn.nextSibling,
  );
  if (searchPatientBtn) {
      ...
      try {
        ...
        if (data.bloqueado) {
          ...
          if (data.motivoBloqueo === "NO_DP") {
            mensaje =
              "❌ Este afiliado no tiene un Día Preventivo previo. No puede acceder a consultas extramodulo.";
          ...
          bloqueDiv.className =
            "bg-red-50 border border-red-300 rounded-lg p-4 mt-4";
          bloqueDiv.innerHTML = `
        ...
        if (data.alertas?.length > 0) {
          ...
          alertasDiv.className =
            "bg-yellow-50 border border-yellow-300 rounded-lg p-4 mt-4";
          let html =
            '<p class="font-bold text-yellow-800 mb-2">⚠️ Alertas Clínicas:</p>';
          data.alertas.forEach((a) => {
            const color =
              a.tipo === "URGENTE" ? "text-red-600" : "text-yellow-700";
  ...
  btnHistorial.addEventListener("click", async () => {
    ...
    btnHistorial.innerHTML =
      '<i class="fas fa-spinner fa-spin mr-2"></i>Buscando en Consultas Extramódulos...';
    ...
    try {
      ...
      if (data.success && data.historial.length > 0) {
        ...
        alert(
          "Este paciente no tiene consultas registradas en la base de Extramódulos.",
        );
      }
    } catch (error) {
      console.error(error);
  ...
  function clearPatientInfo() {
    ...
    // Agregar esto:
    document
      .querySelectorAll(".bloqueo-div, .alertas-div")
    ...
    verEstudiosBtn.addEventListener("click", async () => {
      ...
      if (estudiosContainer)
        estudiosContainer.innerHTML =
          '<p class="text-gray-600 p-4"><i class="fas fa-spinner fa-spin mr-2"></i> Cargando estudios...</p>';
      ...
      try {
        ...
        if (result.success && result.estudios && result.estudios.length > 0) {
          ...
          if (estudiosContainer)
            estudiosContainer.innerHTML = `<p class="text-gray-500 italic p-4">No se encontraron estudios previos.</p>`;
        }
      } catch (error) {
        if (estudiosContainer)
          estudiosContainer.innerHTML =
            '<p class="text-red-500">Error al cargar estudios.</p>';
      }
    });
  }
  ...
  if (estudiosContainer) {
    ...
    <p class="text-sm text-gray-600 mb-4">Fecha: ${study.Fecha || "No disponible"} | Prestador: ${study.Prestador || "N/A"}</p>`;

          // Resultado simple (mamografía, VCC, odontología, etc.)
          if (study.Resultado) {
            modalHtml += `<p class="mb-3"><strong>Resultado:</strong> ${study.Resultado}</p>`;
          }
          if (study.Observaciones) {
            modalHtml += `<p class="mb-3"><strong>Observaciones:</strong> ${study.Observaciones}</p>`;
          }

          // Resultados estructurados (laboratorio, enfermería)
          const resultados =
            study.ResultadosLaboratorio || study.ResultadosEnfermeria;
          if (resultados) {
            modalHtml += `<table class="min-w-full text-sm">`;
            for (const [key, value] of Object.entries(resultados)) {
              if (value && value !== "N/A" && value !== "") {
                const label = key.replace(/_/g, " ");
                modalHtml += `<tr class="border-b"><td class="py-1 font-semibold text-gray-700 capitalize pr-4">${label}:</td><td class="py-1 text-gray-900">${value}</td></tr>`;
              }
            }
            modalHtml += `</table>`;
          }
          ...
          // PDFs
          const links =
            study.LinksPDF?.filter((l) => l) ||
            (study.LinkPDF ? [study.LinkPDF] : []);
          if (links.length > 0) {
            modalHtml += `<div class="mt-4 space-y-2">`;
            links.forEach((link, i) => {
              modalHtml += `<a href="${link}" target="_blank" class="inline-block bg-blue-600 text-white px-4 py-2 rounded text-sm hover:bg-blue-700 transition mr-2">
            <i class="fas fa-file-pdf mr-1"></i>Ver PDF ${links.length > 1 ? i + 1 : ""}
        </a>`;
            });
            modalHtml += `</div>`;
          }
  ...
  if (saveConsultationBtn) {
    saveConsultationBtn.addEventListener("click", async () => {
      const payload = {
        ...
        Profesional: window.dpProfesional
      ...
      if (!payload.DNI || !payload.Apellido || !payload["motivo de consulta"]) {
        alert(
          "Complete los campos obligatorios del paciente y el motivo de consulta.",
        );
        return;
      }

      saveConsultationBtn.disabled = true;
      saveConsultationBtn.innerHTML =
